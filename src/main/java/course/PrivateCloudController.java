...
public class PrivateCloudController {
    ...
    public PrivateCloudController(String mongoURIString) throws IOException {
        ...
        servInst = new ServiceInstance(new URL(SJSULAB.getVCenterURL()), SJSULAB.getVCenterLogin(), SJSULAB.getVCenterPassword(), true);
        if (servInst == null)
        {
            ...
            public void doHandle(Request request, Response response, Writer writer) throws IOException, TemplateException {
                ...
                if (request.queryParams("Create") != null) {
                    ...
                    VirtualMachine vm = (VirtualMachine) new InventoryNavigator(
                            Path).searchManagedEntity("VirtualMachine", vm_template);

                    VirtualMachineRuntimeInfo vmri = vm.getRuntime();

                    HostSystem hs = new HostSystem(vm.getServerConnection(), vmri.getHost());

                    Datacenter dc = (Datacenter) new InventoryNavigator(Path).searchManagedEntity("Datacenter", "T03-DC");
    ...
    public void powerOn(String vmname){
        try {
            VirtualMachine vm = (VirtualMachine) new InventoryNavigator(
                    Path).searchManagedEntity("VirtualMachine", vmname);
